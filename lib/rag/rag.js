...
module.exports = {
    query: async function(userQuery, model, instructions, schema) {
        let userQueries = [];
        userQueries.push({ role: 'user', content: userQuery });

        let exampleQueries = await mongo.getExampleQueries();
    
        exampleQueries.map(i => delete i._id)
    
        const response = await queryWorkflow(userQuery, userQueries, exampleQueries, model || defaultModel, instructions, schema);
        return response;
    },
...
async function generateMongoDBQuery(naturalLanguageQuery, exampleQueries, model, instructions, schema) {
     let content = instructions.replace('${exampleQueries}', JSON.stringify(exampleQueries)).replace('${schema}', JSON.stringify(schema));
...
async function queryWorkflow(naturalLanguageQuery, naturalLanguageQueries, exampleQueries, model, instructions, schema) {
    //console.log(naturalLanguageQuery);
    ...
    try {
        const result = await mongo.executeMongoDBQuery(mongoQuery.query);
