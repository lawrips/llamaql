...
module.exports = {
    query: async function(userQuery, model, instructions, schema, dbName) {
        ...
        let exampleQueries = await service.getAll(dbName);
    
        exampleQueries.map(i => delete i._id)
    
        let query = await generate(userQuery, exampleQueries, model || defaultModel, instructions, schema);
        if (query.error) {
...
async function generate(naturalLanguageQuery, exampleQueries, model, instructions, schema) {
