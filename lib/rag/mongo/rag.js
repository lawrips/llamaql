...
module.exports = {
    getQueries: async function (dbName) {
        ...
        let queries = await db.collection('queries').find({}).toArray();
    ...
    query: async function(userQuery, model, instructions, schema, dbName) {
        ...
        let queries = await mongo.getAll(dbName);
    
        queries.map(i => delete i._id)
    
        const response = await queryWorkflow(userQuery, userQueries, queries, model || defaultModel, instructions, schema, dbName  );
        return response;
    },
...
async function generateMongoDBQuery(naturalLanguageQuery, queries, model, instructions, schema) {
...
async function queryWorkflow(naturalLanguageQuery, naturalLanguageQueries, queries, model, instructions, schema, dbName) {
    //console.log(naturalLanguageQuery);
    //console.log(JSON.stringify(mongoQuery));
    let mongoQuery = await generateMongoDBQuery(naturalLanguageQuery, queries, model, instructions, schema);
    if (mongoQuery.error) {
