...
export const executeNLQuery = async (model, appName, query, annotation, instructions, dataSchema, dataExplanation, generate) => {
  const res = await fetch(`/api/protected/app/${appName}/query?model=${model}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ input: query, annotation: annotation, instructions: instructions, schema: `${dataSchema}\n${dataExplanation}`, generate: generate }),
  });
  return await res.json();
};
