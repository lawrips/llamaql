...
function extractCode(text) {
    const regex = /```(?:javascript|json|js|sql)\s*([\s\S]*?)\s*```/;
...
module.exports = {
    ...
    query: async function (userQuery, model, instructions, schema, dbName, requery) {
        let userQueries = [];
        ...
        let response;
        if (!requery) {
            response = await queryWorkflow(userQuery, queries, model || defaultModel, instructions, schema, dbName);
        }
        else {
            response = await requeryWorkflow(userQuery, queries, requery, model || defaultModel, instructions, schema, dbName);
        }
        return response;
    },
...
async function queryWorkflow(naturalLanguageQuery, queries, model, instructions, schema, dbName) {
    ...
    try {
        ...
        const result = await requeryWorkflow(naturalLanguageQuery, queries, dbQuery.query, model, instructions, schema, dbName, ex);
        return { query: dbQuery.query, data: result, error: null };
    }
}

async function requeryWorkflow(naturalLanguageQuery, queries, query, model, instructions, schema, dbName, previousEx) {
    console.log('Performing requery workflow')

    // if no error is passed in, we'll run this and if there's an error we'll rerun
    if (!previousEx) {
        let dbQuery = await retryQuery(naturalLanguageQuery, queries, query, model, instructions, schema, previousEx);
...
async function retryQuery(naturalLanguageQuery, queries, query, model, instructions, schema, ex) {
