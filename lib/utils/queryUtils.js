export const fetchInitialOptions = async (appName) => {
  ...
  export const executeNLQuery = async(model, appName, query, annotation, instructions, dataSchema) => {
    const res = await fetch(`/api/query?model=${model}&app=${appName}`, {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ input: `${query} (${annotation})`, instructions: instructions, schema: dataSchema }),
    }); 
    return await res.json();
  };
