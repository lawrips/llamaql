...
class Rag {
    ...
    async queryStreaming(userQuery, annotation, model, instructions, schema, generate, streamCallbacks, parallelRequeries = 2) {
        ...
        if (!generate) {
            ...
            try {
                let response = await this.#queryWorkflow(userQuery, annotation, queries, defaultModel, instructions, schema, false, null, streamCallbacks);
                
                // Send the 'query-executed' status
                if (streamCallbacks && streamCallbacks.onQueryExecuted) {
                    console.log('Calling onQueryExecuted with response:', response);
            ...
            let chosenSQL = await this.#chooseQuery(userQuery, annotation, queries, advancedModel[0], schema, responses, streamCallbacks);
            let result = [];
            try {
                            ...
                            if (line !== '[DONE]') {
                                ...
                                if (parsedChunk && parsedChunk.choices && parsedChunk.choices[0].delta) {
                                    ...
                                    if (content && content.trim() !== "") {
                                        fullText += content;
                                        // Send chunk to client via streamCallbacks
                                        if (streamCallbacks && streamCallbacks.onProgress) {
