...
class Rag {
    ...
    async queryStreaming(userQuery, annotation, model, instructions, schema, generate, streamCallbacks, parallelRequeries = 3) {
        ...
        if (!generate) {
            ...
            let queryLogic = {};
            let chosenQuery = await this.#chooseQuery(userQuery, annotation, queries, model, schema, responses, streamCallbacks);
            ...
            try {
                ...
                const finalResult = { query: queryLogic.query, choice: queryLogic.choice, changes: queryLogic.changes, evaluation: queryLogic.evaluation, chat: chosenQuery.chat, data: result, error: null };
                ...
                console.log(ex);
                const errorResult = { query: chosenQuery.query, chat: chosenSQL.chat, data: null, error: ex };
    ...
    async #queryWorkflow(naturalLanguageQuery, annotation, queries, model, instructions, schema, requery, previousEx, streamCallbacks = null) {
        ...
        if (requery) {
            ...
            if (dbQuery.error) {
                ...
                return { query: dbQuery.query, evaluation: null, chat: dbQuery.chat, data: null, error: dbQuery.error };
            }
        }
        let queryLogic = {};
        ...
        try {
            ...
            return { query: queryLogic.query, evaluation: queryLogic.evaluation,chat: dbQuery.chat, data: result, error: null };
        } catch (ex) {
            console.log(ex);
            ...
            if (!requery || (requery && !previousEx)) {
                ...
                return { query: queryLogic.query, evaluation: queryLogic.evaluation, chat: dbQuery.chat, data: null, error: ex };
            }
        }
    }
    ...
    async #chooseQuery(naturalLanguageQuery, annotation, queries, model, schema, responses, streamCallbacks) {
        ...
        for (let i = 0; i < responses.length; i++) {
            ...
            let resultString = JSON.stringify(responses[i].data);
            if (resultString.length > maxTokens) {
                resultString = resultString.substring(0, Number.parseInt(maxTokens / 10)) + '... (truncated)';
            }
            messages.push(
                {
                    role: 'user',
