...
//const advancedModel1 = 'qwen/qwen-2.5-72b-instruct'
//const advancedModel2 = 'qwen/qwen-2.5-72b-instruct'
const advancedModel1 = 'anthropic/claude-3.5-sonnet:beta'
const advancedModel2 = 'openai/gpt-4o-2024-08-06'
//const advancedModel2 = 'openai/gpt-4o-2024-08-06'
//const advancedModel1 = 'anthropic/claude-3-haiku';
//const advancedModel2 = 'anthropic/claude-3-haiku';
...
class Rag {
    ...
    async query(userQuery, annotation, model, instructions, schema, generate, parallelRequeries = 2) {
        ...
        // generation mode - more complex flow
            ...
            for (let i = 0; i < parallelRequeries; i++) {
                queryPromises.push(this.#queryWorkflow(userQuery, annotation, queries, i % 2 == 0 ? advancedModel1 : advancedModel2, instructions, schema, false, null));
            }

            // Execute all attempts in parallel
            responses = await Promise.all(queryPromises);

            let chosenSQL = await this.#chooseQuery(userQuery, annotation, queries, advancedModel1, schema, responses)
            let result = [];
            try {
    ...
    async chat(userQuery, messages, instructions, schema, model) {
        ...
        messages.forEach((message) => {
            aiContent.messages.push({
                role: 'assistant',
                content: message.dbQuery
            });

            aiContent.messages.push({
                role: 'user',
                content: `Here's the result of running your query:\n${JSON.stringify(message.dbResult)}`
            })

            aiContent.messages.push({
                role: 'user',
                content: `My feedback on this query. Reflect on what went wrong and provide and explanation.
    ...
    async createExamples(instructions, schema, data, model) {
        ...
        if (provider == 'openai') {
            ...
            return JSON.parse(processCode(formattedResponse.choices[0].message.content.trim(), 'json', 'extract'));
        }
    }
