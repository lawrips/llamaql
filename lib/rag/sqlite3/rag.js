...
class Rag {
    ...
    async translateStreaming(query, instructions, data, model, onChunk) {
        ...
        try {
            await processStreamingResponse(response, onChunk, model);
        } catch (error) {
    ...
    async #generateDBQuery(naturalLanguageQuery, annotation, queries, model, instructions, schema, onChunk) {
        ...
        if (provider == 'openrouter') {
            ...
            let fullText = await processStreamingResponse(response, onChunk, model);
...
async function processStreamingResponse(response, onChunk, model) {
    let fullText = '';
    let buffer = '';
    ...
    try {
            ...
            for (const line of lines) {
                if (line.startsWith('data: ')) {
                    ...
                    try {
                        ...
                        if (chunkContent) {
                            onChunk({ content: chunkContent, status: 'in-progress' });
        ...
        if (buffer) {
            onChunk({ content: buffer, status: 'in-progress' });
            fullText += buffer;
        }
        onChunk({ content: '', status: 'completed' });
