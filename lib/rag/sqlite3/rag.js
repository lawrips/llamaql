    ...
    async #generateDBQuery(naturalLanguageQuery, annotation, queries, model, instructions, schema) {
        ...
        if (provider == 'openai') {
            ...
            if (formattedResponse.error) {
                return ({ query: '', chat: formattedResponse.error.message, error: formattedResponse.error.message });
            }
        ...
        let message = {
            ...
            messages: [
                ...
                ...(ex ? [{
                    ...
                    content: [{
                        type: "text",
                        text: "Finally, some information on the exception that occurred when attempting to run the query: " + ex.message?.toString(),
                    }]
                }] : []), // Conditional spread to include the annotation if it exists
