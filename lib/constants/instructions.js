...
Don't use markdown, just use regular text. 
`,

chooseInstructions: 
`the following queries were created by various AI engines in order to create SQL that would answer the users original natural language query.
Your job is to look at each query individually and pick the one that you think will best accomplish the user's goals. 

In evaluating, look at the original query, the annotations the user provided, and then for each of the examples:
- the SQL that the AI generated and 
- the result of executing that SQL 

In your response, reply back simply with the number corresponding to the best query. No other information.

As additional context, the DB Schema is:
{schema}.
